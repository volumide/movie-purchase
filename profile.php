<?php
	...
	if ($result->num_rows > 0) while ($rows = $result->fetch_assoc()){
		if ($id){
			...
			?>
				<div class="py-10">
					<h1 class="sm:w-3/6 mx-auto text-4xl text-center font-semibold pb-11"> <?php echo $rows['fullname']."'s Profile" ?> </h1>
				</div>
				<form action="" method="POST" class="sm:w-2/5 mx-auto">
					...
					<div class="py-4">
						<label for="country" class="py-4 font-semibold">Country</label>
					...
					<button type="submit" id="update" name="submit" style="display: none;" class="py-4 px-4 bg-blue-800 text-white rounded w-full font-bold">Update</button>
